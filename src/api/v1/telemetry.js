...
const { checkReplay } = require('../../middleware/replayProtection');

// Helper to populate flags from verification details
function buildFlags(details) {
  const flags = [];
  if (!details) return flags;
  
  if (details.physicsCheck && !details.physicsCheck.pass) {
    flags.push('PHYSICS_VIOLATION');
    if (details.physicsCheck.reason) flags.push(details.physicsCheck.reason);
  }
  if (details.temporalCheck && !details.temporalCheck.pass) {
    flags.push('TEMPORAL_ANOMALY');
  }
  if (details.environmentalCheck && !details.environmentalCheck.pass) {
    flags.push('ENVIRONMENTAL_ANOMALY');
  }
  if (details.trustScore && details.trustScore < 0.7) {
...
function buildFlags(details) {
  ...
  if (details.physicsCheck && !details.physicsCheck.pass) {
    flags.push('PHYSICS_VIOLATION');
    if (details.physicsCheck.reason) {
      flags.push(details.physicsCheck.reason);
    }
  }
...
// Apply API key authentication (skip for health/rules)
router.use((req, res, next) => {
  if (req.path === '/health' || req.path === '/rules') {
    return next();
  }
...
router.post('/', async (req, res) => {
  try {
    ...
    // P1: Replay Protection
    const replayResult = await checkReplay(plant_id, device_id, readings?.timestamp);
    ...
    // P1: Replay Protection (Redis deduplication)
    const { checkReplay } = require('../../middleware/replayProtection');
    const replayResult = await checkReplay(plant_id, device_id, readings?.timestamp);
    if (replayResult.isDuplicate) {
      return res.status(409).json({
    ...
    const response = {
      ...
      verification_details: result.verificationDetails
        ? {
            ...
            flags: buildFlags(result.verificationDetails)
          }
        : null
    };
