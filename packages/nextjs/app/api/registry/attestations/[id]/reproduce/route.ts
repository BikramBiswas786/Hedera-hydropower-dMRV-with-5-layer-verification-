...
/** Re-runs the verification engine on the readings published to HCS and compares every figure. */
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const id = intParam((await params).id, 0, Number.MAX_SAFE_INTEGER);
    return NextResponse.json(await reproduceAttestation(await getAttestation(id)));
  } catch (error) {
